...
An important component of relational data is the *keys*.  These are unique identifiers for a particular datum from a table.  In each of these examples the variable (obviously named) `Key` is what is called a *Primary Key* because it uniquely identifies each row.  You can verify this by counting the number of entries and then filtering only for ones with 2 or more instances.
